...
function cpd(AEσb::ReactionRateParams,     # bridge breaking reaction: £ →£⋆
    ...
    elseif metaplast_model == :original
        return metaplast_percolation_model_orig(Tfun, Pfun, mpar.ma, input...,
...
# metaplast model original version (closer in structure to original code, but
# mass conservation is not ensured)
"""
...
function metaplast_percolation_model_orig(Tfun, Pfun, ma, r, σ, c₀, δvec, £vec, gvec,
    ...
    for i = 2:num_tsteps
        ...
        # (which does not contribute to light gas generation).  @@ Note: this
        # shoul be in line with the treatment of fgas in the original code, but
        # may not accurately reflect tar depletion, as fgas[i] now represents
        # all the gas produced up to and until timestep i, considering that the
        # evacuated gas was _always_ absent.  When we take the difference with
        # fgas[i-1], we then substract a quantity that was computed based on the
        # assumption that the evacuated gas _at_that_timestep_ was always
        # absent.
        ...
        # @@ Is the accounting here consistent? It seems that we want to remove the
...
function metaplast_percolation_model_modif(Tfun, Pfun, ma, r, σ, c₀, δvec, £vec, gvec,
    ...
    for ix = 1:num_tsteps
        ...
        #                                             # but appears to have some conceptual issues, since
        #                                             # fgas[i] now represents all the gas produced up to
        #                                             # and until timestep i, under the assumption
        #                                             # that the total mass equaled _remaining_mass_
